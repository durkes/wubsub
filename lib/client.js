...
module.exports = function ({ url = 'ws://localhost:3000/', retries = 10, heartbeat = 30000 } = {}, onError) {
    ...
    function connect() {
        ...
        // the server's interval plus a buffer for latency and stalls
        ...
        ws.on('message', (data) => {
            // traffic counts as proof of life, same as a ping
            resetHeartbeat();
            ...
            // a callback may unsubscribe, so walk a snapshot but skip what it removed
            let failure;

            for (const subscriber of subscriptions.slice()) {
                if (subscriber[0] === data.t && subscriptions.includes(subscriber)) {
                    try {
                        subscriber[1](data.m);
                    }
                    catch (error) {
            ...
            // raised after delivery so one bad callback cannot cost the others their message
            if (failure) {
                report(failure);
            }
        });
    ...
    function publish(topic, message, callback) {
        ...
        // the server drops a frame with no topic
        if (topic === undefined || topic === null) {
            callback(new Error('A topic is required when publishing'), { error: true });
    ...
    function subscribe(topic, callback) {
        ...
        if (topic === undefined || topic === null) {
            throw new Error('A topic is required when subscribing');
        }
    ...
    function close(code, reason) {
        ...
        // a closed client delivers nothing more, the frame being dispatched included
